...
見本は `rain/`（雨の景色。自然の風景）と `neon/`（ネオン。人工物が並ぶ街）。**新しいワールドは近いほうをコピーして始める。**
...
- **別のワールドへの入口** `add_portal(位置, 次のシーン, フェードの色, 光, 発光材質, 光の暈, 光り始める距離, 入る距離)`。近づくほど光が強くなり、触れると移動する（雨の灯り・ネオンのゲートがこれ）
- **スマホのタッチ操作**（左スティック / 右ドラッグ視点 / 2 本指ズーム / ボタン）
- **画質の反映**（`render_scale`、影、アンチエイリアス）。ワールド固有の軽量化は `_on_quality_changed(low)` を上書きして書く（雨は粒とシェーダー、ネオンは光源の数とシェーダー）
