...
000. **4 回目の感想への対応**: 世界の端を三段構えで塞いだ（端の山並み → 見えない壁 → 落ちたら出現地点に戻す。すべて `WorldBase` なので全ワールド共通）。雨の景色の灯りに近づくと光が強くなり、白くフェードして次のワールド（当面は灰色の世界）へ移るようにした。headless テストに「世界の端」「落下の保険」「灯りからの移動」「灯りの光が強くなる」を追加
...
- クラウドの確認手段（追加分）:
  ...
  - `godot --headless --path game res://tools/world_test.tscn` … ワールド固有の仕掛け（今は灯りからのワールド移動）
...
1. 本キャラのポンチョに骨の鎖を仕込み、SpringBoneSimulator3D で揺らす検証（`build_traveler_apose.py` に裾の骨を足す）
1b. 雨の景色に建物などの小物を増やす（小屋・柵・街灯）
...
4. **決めてほしいこと**
   - 2 つ目に作るワールドはどれか（候補: 深海 / 夕焼け / 淡い空の空間 / ネオン / ビル群 / とにかく広い空間）。灯りの移動先をそれに差し替える
   - 世界の端まで歩いて、山並みの見え方が世界観に合っているか
   - 一人称切替のキーは V でよい？
