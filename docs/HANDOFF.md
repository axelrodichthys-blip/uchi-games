...
- **https://axelrodichthys-blip.github.io/uchi-games/** （動作確認済み。main に push すると 1〜2 分で自動更新される）
  ...
  - 出現地点の**右前 20m** に急斜面の山（約60度）がある。登ろうとすると滑り落ちる。登れない斜面は地面が茶色っぽく表示される
  - F1 の camera_collision_mode で、カメラが小物をすり抜けて透過する方式（既定）と引き寄せ方式を切り替えられる
...
- 手足・コート付きの仮キャラ（黒い身体、サンドベージュの帽子と前が割れたコート）。`game/scenes/player/traveler.gd` がプリミティブで組み立て、待機（呼吸・揺れ）/ 歩き / 走り（前傾・大きな振り）/ ジャンプ（脚を上げる）/ 着地（沈み込み）を手続きで動かす。Mixamo の本キャラが入るまでのつなぎ
...
- 仮キャラの見た目は本キャラのたたき台。手足の長さや帽子の形は `traveler.gd` の定数で変えられる
- 仮キャラのアニメは手続き（数式）なので、Mixamo のアニメを入れたら `traveler.gd` ごと差し替える
- Windows 側の Godot / Blender のパスは未設定

## 次にやること

1. ユーザーに仮キャラの動き・カメラ方式・急斜面の感想をもらう
2. 「歩く気持ちよさ」の初手: 足音、歩行時のカメラの微かな揺れ、加減速の味付け
3. 振り向きの動き（その場で向きを変えるときの足の踏み替え）を仮キャラに足す
...
3. **決めてほしいこと**
   - カメラ: 「すり抜けて透過」と「引き寄せ」のどちらが好みか（F1 で切替）
   - 仮キャラの体型（頭身・手足の長さ・帽子の大きさ）の方向性
   - 一人称切替のキーは V でよい？
